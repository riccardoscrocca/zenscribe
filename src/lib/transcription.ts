    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error('[transcribeAudio] Errore risposta:', errorData);
          errorMessage = errorData.error || errorData.details || '';
        } catch (e) {
          console.error('[transcribeAudio] Errore nel parsing della risposta:', e);
          errorMessage = await response.text();
        }
        throw new Error(`Errore trascrizione (${response.status}): ${errorMessage || response.statusText}`);
      }

      console.log('[transcribeAudio] Parsing della risposta...');
      let responseData: any;
      try {
        responseData = await response.json();
      } catch (e) {
        console.error('[transcribeAudio] Errore nel parsing JSON della risposta:', e);
        const textResponse = await response.text();
        console.log('[transcribeAudio] Risposta testuale:', textResponse.substring(0, 100));
      ...
      console.error('[transcribeAudio] Errore durante la chiamata:', error);
      throw error;
    }
  } catch (error) {
