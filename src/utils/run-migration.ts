...
// La migrazione SQL per il rinnovo automatico delle sottoscrizioni
const SUBSCRIPTION_RENEWAL_MIGRATION = `
/*
...
// SQL per verificare l'estensione pg_cron
const CHECK_CRON_EXTENSION = `
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      -- Se non è possibile creare l'estensione, continua comunque
      RAISE NOTICE 'Impossibile creare l''estensione pg_cron: %', SQLERRM;
    END;
  END IF;
END $$;
`;
...
/**
 * Esegue direttamente SQL sulla console di amministrazione
 */
async function executeSQL(sql: string) {
  try {
    console.log('Esecuzione di SQL personalizzato...');
    
    // Esegui il SQL
    const { error } = await supabase.rpc('pg_execute', { sql_query: sql });
    
    if (error) {
      console.error('Errore durante l\'esecuzione del SQL:', error);
    ...
    console.error('Errore imprevisto durante l\'esecuzione del SQL:', err);
    return { 
      success: false, 
...
// Funzione per eseguire la migrazione di rinnovo automatico delle sottoscrizioni
export async function runSubscriptionRenewalMigration() {
  try {
    // Prima verifica se l'estensione pg_cron è disponibile
    await executeSQL(CHECK_CRON_EXTENSION);
    
    // Esegui la migrazione principale
    return executeSQL(SUBSCRIPTION_RENEWAL_MIGRATION);
  } catch (err) {
