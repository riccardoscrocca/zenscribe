...
BEGIN
  ...
  IF NEW.duration_seconds IS NULL OR NEW.duration_seconds <= 0 THEN
    v_error_message := format('Invalid duration_seconds value: %', NEW.duration_seconds);
  ...
  BEGIN
    ...
    IF v_user_id IS NULL THEN
      v_error_message := format('User not found for patient_id: %', NEW.patient_id);
      INSERT INTO minutes_calculation_log (
        consultation_id,
        duration_seconds,
        calculated_minutes,
  ...
  BEGIN
    ...
    IF v_plan_id IS NULL THEN
      ...
      IF v_plan_id IS NULL THEN
        v_error_message := format('No plan found for user_id: % and free plan not available', v_user_id);
        INSERT INTO minutes_calculation_log (
          consultation_id,
          duration_seconds,
          calculated_minutes,
    ...
    END IF;
  EXCEPTION WHEN OTHERS THEN
    v_error_message := format('Error getting plan details: %', SQLERRM);
    INSERT INTO minutes_calculation_log (
      consultation_id,
      duration_seconds,
      calculated_minutes,
  ...
  BEGIN
    ...
    IF v_subscription_id IS NULL THEN
      v_error_message := format('Failed to get or create subscription for user_id: %', v_user_id);
      INSERT INTO minutes_calculation_log (
        consultation_id,
        duration_seconds,
        calculated_minutes,
    ...
    END IF;
  EXCEPTION WHEN OTHERS THEN
    v_error_message := format('Error managing subscription: %', SQLERRM);
    INSERT INTO minutes_calculation_log (
      consultation_id,
      duration_seconds,
      calculated_minutes,
  ...
  BEGIN
    ...
    RAISE LOG 'Minutes calculation: consultation=%, seconds=%, minutes=%, user=%, subscription=%, old_total=%, new_total=%',
      ...
      (COALESCE(v_old_minutes, 0) + v_minutes);
  EXCEPTION WHEN OTHERS THEN
    v_error_message := format('Error updating subscription minutes: %', SQLERRM);
    INSERT INTO minutes_calculation_log (
      consultation_id,
      duration_seconds,
      calculated_minutes,
